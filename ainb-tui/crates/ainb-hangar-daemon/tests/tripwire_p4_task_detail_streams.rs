...
//! resolved to its roster name (`claude-agent`, never the raw `agent:agent-1`
//! ref: crisp B1, defect 8) and the `Status` field (POSITIVE), paired with a
//! NEGATIVE check that the issue-list `Todo (3)` chrome is gone (we actually
//! navigated).
...
fn task_detail_opens_for_selected_issue() {
    ...
    let detail = sess
        .poll_capture(Instant::now() + Duration::from_secs(15), |c| {
            c.contains("Assignee") && c.contains("claude-agent")
        })
        .expect("task detail never rendered");

    // POSITIVE: sidebar fields bound to the seeded issue/agent, the assignee
    // resolved to its roster name. NEGATIVE: the issue-list status-group header
    // is gone, so we genuinely left the list.
    assert!(
    ...
    assert!(
        ...
        "raw actor ref leaked onto the header:\n{detail}"
    );
    assert!(
