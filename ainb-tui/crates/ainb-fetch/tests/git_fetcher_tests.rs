...
fn clones_local_remote_via_file_url() {
    ...
    let fetched = GitFetcher::new()
        .fetch(&uri, "local-test", cache.path())
        .expect("fetch");

    assert!(fetched.path.exists(), "fetched dir missing: {:?}", fetched.path);
    assert!(
        fetched
            .path
            .ancestors()
            .any(|p| p == cache.path().join("local-test")),
        "fetched path not under cache/<source-name>: {:?}",
        fetched.path
    );

    let greeting: PathBuf = fetched.path.join("greeting.txt");
    let content = std::fs::read_to_string(&greeting).unwrap_or_else(|e| {
        panic!("greeting.txt missing under {:?}: {e}", fetched.path)
    });
...
fn second_fetch_is_idempotent() {
    ...
    let first = GitFetcher::new()
        .fetch(&uri, "idem", cache.path())
        .unwrap();
    let second = GitFetcher::new()
        .fetch(&uri, "idem", cache.path())
        .unwrap();
