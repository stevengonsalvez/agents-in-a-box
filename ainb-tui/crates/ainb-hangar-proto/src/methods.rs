...
/// read, workspace-scoped; with a `board_id` the issue must also be a card on
/// that board (the Boards overlay), without one any issue in the workspace
/// resolves (the task-detail backfill, crisp B1).
