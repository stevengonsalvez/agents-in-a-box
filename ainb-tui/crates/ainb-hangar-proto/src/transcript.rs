...
/// view reads the same under both executors. The card is not that view: it
/// labels every chunk kind, and a labelled row with an empty body reads as if
/// the agent said nothing. So the card asks here for exactly these two.
///
/// Scrubbed before the [`BODY_MAX`] cut, like every classified body: an
/// operator pastes credentials into prompts too.
#[must_use]
    ...
    let text = match event_type {
        ...
        "acp.usage" => {
            ...
            match (
                ...
                (Some(amount), Some(currency)) => format!("{tokens} · {amount} {currency}"),
                _ => tokens,
            }
        }
        _ => return None,
    };
    Some(truncate_chars(&scrub(&text), BODY_MAX))
}
