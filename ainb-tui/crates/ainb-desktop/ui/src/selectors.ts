...
import { attentionRows, elsewhereCount } from "./board.ts";
...
export const ROOT_SELECTORS = {
  ...
  /** The daemon's open rows the attention list draws. */
  waitingRows: (store, host) =>
    host === undefined
      ? 0
      : attentionRows(store.section(host, "fleet"), store.section(host, "sessions")).length,
