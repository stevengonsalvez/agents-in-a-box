...
//!
//! Lifted from `crates/ainb-core/src/models/usage.rs::parse_codex_*`.
//! See the Claude module for the rationale on the host-fn rewrite and
//! the dropped append cache.
//!
//! Directory layout: `~/.codex/sessions/<YYYY>/<MM>/<DD>/rollout-*.jsonl`.
...
pub fn parse_dir(sessions_root: &str) -> Vec<ProviderCall> {
    ...
    let years = match host::fs_read_dir(sessions_root) {
        Ok(d) => d,
        Err(host::HostError::NotPermitted) => {
            host::log(
                LogLevel::Warn,
                "session-reader/codex: not permitted to read sessions dir",
            );
            return calls;
        }
        Err(_) => return calls,
    ...
    for year_path in years {
        ...
        let Ok(months) = host::fs_read_dir(&year_path) else {
            continue;
        };
        for month_path in months {
            ...
            let Ok(days) = host::fs_read_dir(&month_path) else {
                continue;
            };
            for day_path in days {
                ...
                let Ok(files) = host::fs_read_dir(&day_path) else {
                    continue;
                };
                for path in files {
                    ...
                    let bytes = match host::fs_read_file(&path) {
                        ...
                        Err(_) => {
                            host::log(
                                LogLevel::Warn,
                                &format!("session-reader/codex: skip unreadable {path}"),
                            );
                            continue;
                        }
                    };
                    let Ok(content) = String::from_utf8(bytes) else {
                        host::log(
                            LogLevel::Warn,
                            &format!("session-reader/codex: skip non-utf8 {path}"),
                        );
                        continue;
                    };
                    ...
                    calls.extend(parse_source(&path, &content));
                }
            }
        }
    }
...
fn is_date_component(path: &str, len: usize) -> bool {
    let basename = path_basename(path);
...
mod tests {
    ...
    fn parses_session_meta_then_token_count() {
        ...
        assert_eq!(calls.len(), 1, "expected one call from token_count");
        let c = &calls[0];
