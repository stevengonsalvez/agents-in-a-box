...
//! 7-line hash drops blake3 + its SIMD intrinsics from the wasm
//! binary, per `reference_inline_fnv_drop_blake3`.
//!
...
pub fn provider_call_id(path: &str, offset: u64) -> u64 {
    // Avoid `format!` to keep wasm size + heap traffic small.
...
mod tests {
    ...
    fn known_values_match_reference() {
        // Cross-checked against the FNV-1a 64 reference implementation.
