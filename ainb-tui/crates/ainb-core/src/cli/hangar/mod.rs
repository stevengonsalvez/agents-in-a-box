...
fn pid_holds_socket(pid: u32, socket: &std::path::Path) -> bool {
...
fn socket_names_match(name: &str, expected: &std::path::Path) -> bool {
...
fn run_daemon_stop() -> Result<()> {
    ...
    match stop_decision(&pid_path, &socket) {
        StopDecision::Signal(owned) => {
            ...
            if died {
                std::fs::remove_file(&pid_path).ok();
                println!("hangar daemon: stopped (signalled pid {pid}, {signal})");
            } else {
                ...
                println!(
                    "hangar daemon: SIGTERM then SIGKILL sent to pid {pid}, still alive after {}s \
                     (pid file kept)",
                    DAEMON_STOP_GRACE.as_secs() + DAEMON_STOP_KILL_GRACE.as_secs()
                );
            }
        }
