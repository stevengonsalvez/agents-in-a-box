//! P8.4 — prove the Kanban card-move key issues the right daemon RPC.
//!
...
//! board (`K`) and pressing `Shift+→` on the focused card, the plugin must issue a
//! `hangar/task_transition` whose `to_status` is the destination column's drop
...
use ainb_plugin_protocol::params::{
    ...
    UnixSocketSendParams, KEY_MOD_SHIFT,
};
...
/// seed-shaped result so the board has a card to move (one queued task).
...
/// Drive one render so the plugin drains its pending kanban action; relay any
...
async fn send_key<W: tokio::io::AsyncWrite + Unpin>(host_write: &mut W, code: KeyCode, mods: u8) {
    let key = KeyEvent {
        code,
        mods,
...
#[tokio::test]
async fn shift_right_issues_task_transition() {
    let body = async {
        let home = tempfile::tempdir().expect("home");
        std::env::set_var("AINB_HANGAR_HOME", home.path());
        let stream_id = format!("sock-kanban-{}", std::process::id());
        ...
        // Open the Kanban board (`K`), then Shift+→ to move the focused queued
        // card to the running column.
        send_key(&mut host_write, KeyCode::Char { ch: 'K' }, 0).await;
        send_key(&mut host_write, KeyCode::Right, KEY_MOD_SHIFT).await;
        ...
        for _ in 0..40 {
            ...
            let hit = seen.lock().unwrap().iter().any(|(m, p)| {
                m == daemon_methods::HANGAR_TASK_TRANSITION
                    && p.get("task_id").and_then(serde_json::Value::as_str)
                        == Some("01HANGARTASKQUEUED01")
        ...
        assert!(
            sent,
            "Shift+→ on the queued card must issue hangar/task_transition(to_status=running); saw: {:?}",
            seen.lock().unwrap()
        );
    ...
    tokio::time::timeout(BUDGET, body)
        .await
        .expect("exceeded kanban transition budget");
}
