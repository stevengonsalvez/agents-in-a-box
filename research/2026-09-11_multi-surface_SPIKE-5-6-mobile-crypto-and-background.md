...
| heartbeats after Home or lock | **none after +1.5 s** in 11 of 11 runs (9 sent none, 2 sent one within 1.5 s); first missed beat 3.6 to 16.5 s after the transition | 0 in 10 of 10 | 40 of 40 in 10 of 10: alive for the whole 600 s |
| socket reset after the transition | none in 600 s (6 runs); reset at 264 to 424 s (5 runs, 2 verified as memory kills) | 6.0 to 6.4 s in 9 of 10; 1 locked run lost every beat with no reset | none in 600 s |
...
| Does D13 need a JS Noise implementation review? | **No.** No JavaScript touched a key, nonce or frame on any target. The TypeScript side only passes key bytes in and reads typed records out. A JS Noise review becomes relevant only if the first-gate iPhone run fails in the crate itself, which the simulator run gives no reason to expect. | `[simulator]`, `[emulator A15]`, `[emulator A14]` |
| Does the push reopen row move to M1+1? (threshold: grace under 60 s) | **Yes.** On iOS no heartbeat left the app later than 1.5 s after Home or lock in any of 11 simulator runs, and the first missed beat came 3.6 to 16.5 s after the transition. JS itself kept running for up to 4.2 s (the `appstate background` mark), then the app was suspended. The grace is therefore a few seconds at most, and effectively zero for the heartbeat `[inference: suspension read from the stopped heartbeat and marks, not from an OS log]`. iOS suspension of a backgrounded app without a background mode is the documented platform model, so this is not a simulator artefact `[inference]`. Android 15 destroyed the socket at 6.0 to 6.4 s in 9 of 10 runs, and the tenth lost every beat. All of these are under 60 s. Android 14 on AC power kept it for 600 s, but M1 cannot rely on an OS version older than the phones it targets. | `[simulator]`, `[emulator A15]`, `[emulator A14]` |
