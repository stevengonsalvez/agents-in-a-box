//! e38.8 — prove the agent-picker Assign key issues the right daemon RPC.
//!
//! Drives the **real** [`HangarPlugin`] behind the **real** SDK [`Server`],
//! with the test playing the host that relays the plugin's reverse
//! `unix_socket_*` calls to a mock daemon recording each `(method, params)`.
//! After opening the picker (`a` on the selected issue-list row), selecting an
//! actor, and pressing Enter, the plugin must issue a `hangar/issue_update`
//! whose `assignee` is the picked actor-ref and whose `issue_id` is the issue
//! the picker was opened for.
//!
//! This is the user-visible proof for the review BLOCKER: opening the picker
//! and pressing Enter must reassign the issue (reach `issue_update` over the
//! socket), not just close the modal.
...
/// a seed-shaped result so the issue list has a row (`issue-1`) and the picker
/// has an actor to assign (`agent:claude-agent`).
...
fn result_for(method: &str) -> serde_json::Value {
    match method {
        ...
        m if m == daemon_methods::HANGAR_AGENTS_LIST => serde_json::json!({
            "actors": [
                {"actor_ref":"agent:claude-agent","display_name":"claude-agent",
                 "subtitle":"agent · gpt5","presence":"online","is_agent":true,
                 "recent_rank":0}
            ]
        }),
...
/// Drive one render so the plugin drains its pending assign action; relay any
...
#[tokio::test]
async fn enter_in_picker_issues_issue_update_assignee() {
    let body = async {
        let home = tempfile::tempdir().expect("home");
        std::env::set_var("AINB_HANGAR_HOME", home.path());
        let stream_id = format!("sock-picker-{}", std::process::id());
        ...
        // Open the agent picker (`a`) on the selected issue, then Enter to assign
        // the selected (only) actor.
        send_key(&mut host_write, KeyCode::Char { ch: 'a' }).await;
        ...
        for _ in 0..40 {
            ...
            let hit = seen.lock().unwrap().iter().any(|(m, p)| {
                m == daemon_methods::HANGAR_ISSUE_UPDATE
                    ...
                    && p.get("assignee").and_then(serde_json::Value::as_str)
        ...
        assert!(
            sent,
            "Enter in the picker must issue hangar/issue_update(assignee=agent:claude-agent); saw: {:?}",
            seen.lock().unwrap()
        );
    ...
    tokio::time::timeout(BUDGET, body)
        .await
        .expect("exceeded agent-picker assign budget");
}
