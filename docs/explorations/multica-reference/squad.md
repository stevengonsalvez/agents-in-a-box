...
|---|---|---|---|---|
| 1 | Leader system-prompt briefing (Operating Protocol + Roster + Instructions) injected at claim time, keyed via `agent_task_queue.squad_id` | **CLOSED** (#5 / #7 / #25 / `7-rest`). `agent_task_queue.squad_id` (mig 0045) keys a claim-time hook that appends `daemon/src/squad_briefing.rs`'s three sections to the run's `CLAUDE.md`: the Operating Protocol, the Squad Roster (leader self-row + one row per non-archived member carrying its free-text `role` and the skills it will actually materialise) and `## Squad Instructions` verbatim. Inspect it with `ainb hangar squad briefing <squad-id>` | Remaining divergences: rows carry `name — <agent\|human> — <id>` rather than multica's mention markdown (gap #2), and the skills segment is materialisation-shaped (both `agent_skill.enabled` and `agent.disabled_runtime_skills` applied) where multica reads one live registry | Done |
...
| 10 | Card-level squad-fan-out reachable via keyboard (assign-squad picker) | **Fixed** (#450): the picker is `s`, not the router-claimed `q` | Done | **S** (reserved-key sets hoisted into `screen/router.rs`; screens rebound off them; invariant test enforces disjointness) |
