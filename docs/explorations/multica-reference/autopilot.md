...
- **Rule versioning + human attribution — LANDED (#14, migration 0061).** `autopilot_rule_version` is an append-only accountability ledger: one row per SUBSTANTIVE publish (create / instructions / schedule / target / policy / pause / resume / trigger), each carrying `published_by` (a canonical `member:<id>` actor ref) and a `config_summary` JSON snapshot of the rule as published. **A cosmetic edit (a rename) mints no row** — the classifier (`ainb_hangar_core::autopilot::rule_version::classify`) is the single definition of "substantive", so a title tweak never re-assigns blame for an unattended run. The edit surface it hangs off (`AutopilotRepo::update_as`, `hangar/autopilot_update`, `ainb hangar autopilot edit`) did not exist before this item either. Runs carry multica's attribution FORK on `autopilot_run.accountable_actor` / `attribution`: an unattended fire (cron / webhook / api) resolves the newest rule version's publisher and stamps `rule_owner`, while a named human's "run now" stamps `direct_human` and names THEM. Deliberately NOT ported: multica's per-trigger publisher (migration 189) — hangar collapses trigger config onto the `autopilot` row, so arming a trigger is a substantive publish on the RULE's chain — and multica's two-column `published_by_type`/`published_by_id` polymorphism, which folds into hangar's established single actor-ref TEXT convention. Deliberately NOT backfilled: a pre-0061 autopilot reads "unversioned" and its old runs read NULL rather than carrying a fabricated audit record.
