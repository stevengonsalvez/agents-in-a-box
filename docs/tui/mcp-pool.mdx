...
Open the **MCP** entry in the home sidebar (or press `m`) for a live overlay of what's served right now — and **which sessions share each backend process**.
...
- **`s` — stop server**: reaps the selected server's process; attached sessions reconnect and the next attach respawns it.
- **`X` — stop pool**: shuts the whole daemon down; every session falls back to its own MCP processes.
