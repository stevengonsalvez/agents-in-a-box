...
- **`i` — import**: pulls stdio servers from the current directory's `.mcp.json` **and** your Claude user scope (`~/.claude.json`) into the project config (`./.ainb/config.toml`), then registers any new ones with the live daemon so they appear in the table immediately — no session restart needed. **`I`** (shift) imports into the **user** config (`~/.agents-in-a-box/config/config.toml`) instead, so every project picks them up. Import is additive: existing entries are never overwritten, and servers whose command doesn't resolve on the host are skipped. The result (`▸ imported …`) shows in a line above the help bar.
...
![ainb MCP pool overlay import — the overlay open over the home screen; pressing `i` imports the project's .mcp.json server into ./.ainb/config.toml, a "▸ imported context7 → …/.ainb/config.toml" result line appears above the help bar, and the context7 row shows in the table; the help bar reads "↑↓ select · s stop server · X stop pool · i import · I import·user · r refresh · esc close"](../assets/screenshots/mcp-import.gif)
