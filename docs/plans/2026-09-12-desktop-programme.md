...
| Phase 2 Versioned sections (wave 3) | 1 | **in review** | Phase 3 (met 2026-09-12) | 19 sections, `SectionVersions` | 117 AppState fields regrouped into the 19 sections behind `Versioned<T>`, one section per commit; `versions()` / `changed_since()` plus `tests/versioned_sections.rs`, whose load-bearing case is that two full draws bump nothing. The plan's `main.rs` redraw rewiring is deliberately NOT here: input already forces a repaint and the app tick repaints every 250 ms, so the version gate adds no trigger, and measured on this branch it costs 6.6x idle CPU because the per-iteration housekeeping takes `&mut` on sections every loop (43 ticks/15 s baseline against 286). |
