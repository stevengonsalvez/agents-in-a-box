...
| P2-P5 effects and hosts | 2 | **P2 and P3 done on v2** (P2 PR #1000 at c599ebd7, P3 PR #1008 at 36e84831, 2026-09-14, four reviews each); P4 in review #1021 (review screens off `main.rs` host writes, #1017: plugin actions and the in-place attach as effects with reports, executor without `AppState`, per-plugin `ui.state` at protocol 0.1.2, command context gate, `core-tripwires` CI job); P5 queued | P1 (met) | per-screen tripwires green; every side effect an `Effect` the host executes; parity fixtures per screen | goal `docs/plans/goals/2026-09-13-p2-p5-effects-and-hosts.md`; P2: `Effect` outbox with attach, detach, editor, OAuth login and clipboard paste, pointer returns an `Intent`, `AppEvent` crate-private (`test-support`), `Args` honoured, `host::TERMINAL` and `RendererHost::columns` gone, side-effect guards in `ainb-app/tests/host_side_effects.rs` |
...
| T0-section: `agent_status` as section 20 | 4 | **done on v2** (PR #1019 at f69e3655, 2026-09-14, three reviews; follow-up #1031 makes the host task the single owner) | P1, T0-daemon (met with follow-ups), #962 (merged as #1014) | a surface renders the Fleet panel from section 20 alone: met in the gate (TestBackend, section-only panel paints the plugin's cells); NOT met in production, where the TUI Fleet panel still renders from the plugin's own read and section 20 has no production consumer (single owner is #1031) | one joined daemon read `fleet/roster_status` (capability `fleet.roster_status.read`) costs 1 projection per read; criterion 9 NOT met per process: the TUI pays 2 whole-Fleet projections per event (plugin read plus host task read), 3 with `legacy_panel`, until #1031 gives section 20 a single owner; an N-1 daemon falls back to the two reads (rows with states, no answer or approval actions); section 20 folds through the shared proto `StatusView`; card words are `AgentState::as_str()` or row fields, no DONE; absent, stale and unreachable render in the lens body; `[fleet.status] legacy_panel` rollback honoured in v1.29.0 (assumed at the v2 cut) and removed with the pre-section read in v1.30.0 |
| W0-mirror per-drain apply, scalar selectors, subscription filter, specta feature | 4 | **in progress** (lane K, `feat/w0-mirror`, goal `2026-09-14-w0-mirror.md`, 2026-09-14); #983 merged as PR #1026, residuals #1032 | **P1 merged**, **#983 met** (goal `docs/plans/goals/2026-09-14-983-redaction-layer.md`: all 47 fields triaged, the 12 high closed at the type level) | bench ceiling 12,800 runs, 32.5 ms per 1,000 frames, units = 125; frames go out only through `ainb_app::wire::section_json(&AppState, SectionId)`, and `ainb-app/tests/state_serde.rs` stays green: name deny-list, type deny-list, canary, token-shape tripwire, and the locked key-path fixture (`ainb doctor --wire-shape` prints the drift). `Serialize` on a section lands behind those tests, never around the seam | spec v1.5 W0-mirror; sweep on PR #982; seam and checks in the #983 PR |
...
| C | `2026-09-12-status-t0.md`, then `2026-09-13-963-tui-connection.md`, now #962 | claude-gcp | **merged** #934, #967, #978, #998, #1014, #1019 (#953, #958, #960, #961, #962, #963, #1015 closed); now #1031 on `fix/1031-single-owner` | relaunched after the gcp reboot, handle in the orchestrator memory |
...
| F | `2026-09-13-p1-ainb-app.md` then `2026-09-13-p2-p5-effects-and-hosts.md` | claude-hetzner | worktree `p1-ainb-app`; **merged** #973, #975, #982 (P1 complete); **merged** #1000 (P2), #1008 (P3); now P4 with #1017 as added criteria | `term_fa0949da-d441-4575-b345-a22d9386a74a` |
...
| K | `2026-09-14-983-redaction-layer.md` then `2026-09-14-w0-mirror.md` | claude-gcp | worktree `g6-verify`; **merged** #1026 (#983 closed, residuals #1032); now W0-mirror on `feat/w0-mirror` | spawned 2026-09-14, handle in the orchestrator memory |
