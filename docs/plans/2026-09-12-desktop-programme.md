...
| T0-daemon: status store on `fleet_session`, one txn per event, retention, normalizers, OSC schema | 4 | **in review** | spike 1 (done), #916 | identical tuples across CLI, web, TUI; silence never `done` | draft PR #934, every criterion met. `ainb-core/tests/status_one_truth.rs` drives one fixture session hook-to-store and asserts the same `(session_key, state, provenance, tier, evidence_observed_at)` from `fleet/status`, the `ainb-plugin-hangar` fleet panel row and `GET /api/needs`. Plus normalizers + `status_unknown_event`, retention, the drift assertion over a 1,000-event replay, the rollback flag, and the spike-8 ceiling at p99 12.1 ms (900 events, 10/s) |
...
| C | `2026-09-12-status-t0.md`: #916 pane binding, then T0-daemon, plus the three red daemon tests from #925 | claude-gcp | `feat/916-pane-binding`, PR #934 ready for review. #916 done, T0-daemon done, all three #925 tests green at root cause. Needs nothing from another lane | `term_49ad9b50-e253-497d-ad2c-62059f021b16` (the idle original was closed 2026-09-12, this is the only lane C session) |
