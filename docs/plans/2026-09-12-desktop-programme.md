...
flowchart TD
  ...
  SK9[spike 9 daemon reuse]:::done --> I916[#916 pane binding]:::done --> T0d
  SK8 --> W0w
  P1 -->|merged| W0m
...
| #916 pane binding without launcher env | 4 | **done** | none | `env -i` hook test, no duplicate legacy row | `tests/pane_binding_916.rs` 4/4 green on `feat/916-pane-binding`; one pane binds and retires its legacy row, zero or two panes give `pane_unbound`, replay leaves one row |
| T0-daemon: status store on `fleet_session`, one txn per event, retention, normalizers, OSC schema | 4 | **in review, one gate blocked** | spike 1 (done), #916 | identical tuples across CLI, web, TUI; silence never `done` | draft PR #934. Store, normalizers + `status_unknown_event`, retention, drift assertion, rollback flag and the spike-8 ceiling (p99 12.1 ms at 900 events, 10/s) all green. The TUI third of the tuple gate is BLOCKED: the fleet panel derives its own state from `SessionFleetMetadata.lifecycle`, which carries no `session_key`, tier, provenance or evidence clock, and putting the daemon's status read on it means editing `app/state.rs` — lane A's file. CLI and web parity is asserted |
...
| C | `2026-09-12-status-t0.md`: #916 pane binding, then T0-daemon, plus the three red daemon tests from #925 | claude-gcp | `feat/916-pane-binding`, draft PR #934. #916 done; all three #925 tests green at root cause; T0-daemon needs one `app/state.rs` change from lane A for the TUI third of the tuple gate | `term_49ad9b50-e253-497d-ad2c-62059f021b16` (the idle original was closed 2026-09-12, this is the only lane C session) |
