...
| S-D concurrency tests (wave 4) | 1 | **in review** | S-A, S-B, S-C, Phase 2 (all met 2026-09-13) | answer race, resize during answer, surface combo | PR #964: `tripwire_multi_attach_resize.rs` (three clients, one resized mid-answer; the `window-size` assertion proven to bite by swapping in `resize-window`) and `surface-combo-smoke.sh` wired into the `ainb-hooks` job. `answer_race.rs` 20/20 as the plan asks; its TUI and web sides were already covered by S-C (`plugin.rs:6685`, `ask-answer.spec.ts:154`) so nothing was duplicated, and its one uncovered item, the `AttentionAnswered` broadcast count, is flagged in lane B's file rather than edited. The smoke found #963: a running TUI never appears in `hangar connections list`, half fixed here (`tui_client`) and half left to S-B |
...
| T0-daemon: status store on `fleet_session`, one txn per event, retention, normalizers, OSC schema | 4 | **done on v2 with declared follow-ups** (PR #934, 2026-09-13; three review passes) | spike 1 (done), #916 (done), 0097 (met) | identical tuples across CLI, web, TUI; silence never `done` | apply transaction with attention projection, retention, rollback flag, normalizer, `fleet/status` capability, the three baseline daemon reds fixed at root (v2 CI fully green at c79e1aca). Follow-ups: #960 D14 columns + incarnation fence, #961 stored binding fingerprint + invalidation, #962 panel on `fleet/status` + TestBackend gate + supersede in the apply txn |
...
| C | `2026-09-12-status-t0.md`: #916 pane binding, then T0-daemon, plus the three red daemon tests from #925 | claude-gcp | `feat/916-pane-binding` **merged PR #934**; next: #960 + #961 (D14 columns, incarnation fence, binding fingerprint) on a fresh branch | `term_49ad9b50-e253-497d-ad2c-62059f021b16` |
