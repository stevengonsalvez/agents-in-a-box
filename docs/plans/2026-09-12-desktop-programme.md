...
| S-C card retirement (wave 2) | 1 | **in review** | S-B on `v2` (met) | `AttentionAnswered` folds in the TUI and the web form closes on `already_answered` | PR #934: `ControlCenterState::retire_answered` plus the wire-level fold test in `plugin.rs`, `app.js` retires the card's controls; copilot already sends `answered_by` and the daemon stamps the host (S-B 3b); plan `:319` |
