...
| P1 `ainb-app` extraction | 2 | **done on v2** in three PRs (lane F, 2026-09-13): P1a service layer (#973, 84f4c9ef), P1b component pure halves (#975, e1e817ad), P1c state machine, `Chord`, `Intent`, `dispatch`, command registry with unbound commands, `RendererHost` (#982) | slice 1 merged (G6 with Stevie) | `cargo test -p ainb-app` runs 281 of the 297 `app/` tests; the other 16 need a ratatui `Frame`, a crossterm event or `UiState` and stay in core. `intent_key_parity` covers all 525 keymap rows. A `Text` intent and a `Command` intent each bump one section. `renderer_free` proves no ratatui or crossterm in the crate. | inventory `docs/plans/2026-09-13-p1-extraction-inventory.md`, which also lists the side effects P2 to P5 turn into `Effect`s. The spec's 41k LOC and 130-signature figures were wrong; the real move is about 120k LOC. `Serialize` on state and `typescript-bindings` were cut from P1c after a security review found 47 sensitive fields, and they land in the mirror phase behind redaction (#983). #976 tracks a duplicate keymap row id. |
...
| T0-daemon: status store on `fleet_session`, one txn per event, retention, normalizers, OSC schema | 4 | **done on v2** (PR #934 at c79e1aca, follow-up PR #967 at 45b41a2d, 2026-09-13) | spike 1 (done), #916 (done), 0097 (met) | identical tuples across CLI, web, TUI; silence never `done` | apply transaction with attention projection, retention, rollback flag, normalizer, `fleet/status` capability; migration 0099 stores tier, clocks and `session_incarnation`, the fence orders on `session_started`, restart closes the old run's cards, the pane binding is written once and invalidated on fingerprint mismatch. Remaining: #962 (panel on `fleet/status`, TestBackend gate, supersede in the apply txn), parked until P1 lands |
| W0-mirror per-drain apply, scalar selectors, subscription filter, specta feature | 4 | planned | **P1 merged**, #983 redaction layer before any AppState serialisation (47 sensitive fields found 2026-09-13) | bench ceiling 12,800 runs, 32.5 ms per 1,000 frames, units = 125; per-section JSON, no whole-state Serialize without the type deny-list, canary test and token-shape tripwire | spec v1.5 W0-mirror; sweep on PR #982 |
| T0-section: `agent_status` as section 20 | 4 | planned | P1, T0-daemon (met with follow-ups), #962 | fleet panel renders from the section | spec v1.5; #962 is the prerequisite (panel on `fleet/status`) |
...
| spikes 5, 6 phone crypto via uniffi, background socket lifetime | 6 | **in progress** (lane G on Stevie's mac `mbp5`, 2026-09-13) | none | measured on real iPhone and Android: handshake, cold start, bundle delta, keychain custody, background socket grace, local banner at 1/5/30 min; decision input for M1 gate wording | goal `docs/plans/goals/2026-09-13-spikes-5-6-mobile.md` |
...
| C | `2026-09-12-status-t0.md`: #916 pane binding, then T0-daemon, plus the three red daemon tests from #925 | claude-gcp | **merged** #934, #967 (#960, #961 closed); now on the flake register (#953, #958) while #962 waits for P1 | `term_49ad9b50-e253-497d-ad2c-62059f021b16` |
...
| F | `2026-09-13-p1-ainb-app.md`: P1, the state machine into `ainb-app`, de-crossterm at the host edge | claude-hetzner | worktree `p1-ainb-app`; **merged** #973 (P1a), #975 (P1b); P1c #982 | `term_fa0949da-d441-4575-b345-a22d9386a74a` |
| G | `2026-09-13-spikes-5-6-mobile.md`: spikes 5 and 6 on real phones | mbp5 (Stevie's mac) | worktree `spikes-5-6` in the `agents-in-a-box-anthias` checkout, from `v2` | `term_6d95d55f-0a25-47ae-80a2-573507efd254` |
