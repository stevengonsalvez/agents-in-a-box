...
| #916 pane binding without launcher env | 4 | **in review** (PR #934) | none | `env -i` hook test, no duplicate legacy row | `pane_binding.rs` 1/0/2 binding unit-tested; candidate query and fingerprint invalidation sent back |
| T0-daemon: status store on `fleet_session`, one txn per event, retention, normalizers, OSC schema | 4 | **in review** (PR #934) | spike 1 (done), #916, W0-wire migration 0097 (met) | identical tuples across CLI, web, TUI; silence never `done` | apply transaction, retention and rollback flag met; D14 migration (tier, provenance, clocks, incarnation) and the surface gate sent back |
...
| C | `2026-09-12-status-t0.md`: #916 pane binding, then T0-daemon, plus the three red daemon tests from #925 | claude-gcp | `feat/916-pane-binding`, PR #934 sent back once (2026-09-12: pane candidate query can bind across agents, authority demotion freezes tier-5 rows, cwd stamp loop, no fingerprint invalidation, gate does not drive the surfaces, D14 migration now due after 0097); baseline daemon reds fixed at root | `term_49ad9b50-e253-497d-ad2c-62059f021b16` |
