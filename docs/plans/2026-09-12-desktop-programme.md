...
| T0-section: `agent_status` as section 20 | 4 | **in review** as PR #1019 on `feat/t0-section` (lane C, #1015) | P1, T0-daemon (met with follow-ups), #962 (merged as #1014) | fleet panel renders from the section | one joined daemon read `fleet/roster_status` (capability `fleet.roster_status.read`, 1 projection per event per surface, was 2); section 20 `AgentStatus` folds it through the shared proto `StatusView`, the reducer the plugin panel also uses; card words are `AgentState::as_str()` or row fields, no DONE; absent, stale and unreachable render in the lens body; `[fleet.status] legacy_panel` rollback honoured in v1.29.0 (first tag carrying section 20, assumed at the v2 cut) and removed with the pre-section read in v1.30.0 |
