...
| D3 review tab, settings | 3 | **started** 2026-09-19 on lane F: goal #1202 and spec amendment #1205 on v2 (the row split, the bounded `git_view` section with its withheld signal, #1162's throughput wording); D3a (the `git_view` byte budget, the palette rows in the reducer behind a golden row set, the parity frames both renderers read) in review on `d3a-seams`; D3b review tab and D3d settings with #1175, the journey and the proof scenario next; goal `docs/plans/goals/2026-09-19-d3-desktop-review.md` | D2 | parity suite for the screens D3 draws, with the mutation check | base spec D3, amended 2026-09-19 |
...
| D4' updater, release matrix (host switcher moved to R1) | 3 | **started** 2026-09-19 on lane S: goal #1220 on v2 (`docs/plans/goals/2026-09-19-d4-updater-release.md`, reviewed, eight decisions folded, the repository move planned as its own section); D4b the release matrix in review first (three desktop bundles under the release version, the pinned bundler, signing only from named Apple secrets, the manifest's `desktop` key, the bundle smoke over real bundles, the rehearsal script); D4a the sidecar seam and the desktop skew leg next, in parallel; D4c the updater after D4b and D3d | D3' for D4c; D4·spec for D4a and D4b | release-branch human-driver run: the install half on D4b's first prerelease, the update half on D4c's | spec v1.3 amendment 26 |
...
| Q | the fix push for P6d #1206, then the P6e goal #1210 | claude-gcp | worktree `p6d-fix`, pushes to `p6d-sessions-daemon`; #1206 reworked to land dark, verified merge after three residuals; P6e goal under revision after a design review | spawned 2026-09-19 through `worktree create --agent claude` |
