...
| P2-P5 effects and hosts | 2 | **in progress** (lane F, four staged PRs, 2026-09-13) | P1 (met) | per-screen tripwires green; every side effect an `Effect` the host executes; parity fixtures per screen | goal `docs/plans/goals/2026-09-13-p2-p5-effects-and-hosts.md` |
...
| spikes 5, 6 phone crypto via uniffi, background socket lifetime | 6 | **Android done on an emulator 2026-09-14, iOS blocked** (lane G, no phone attached, no Xcode; emulator-only by Stevie's call) | none | measured on an Android 15 emulator: handshake, cold start, bundle delta, key custody, background socket grace, local banner at 1/5/30 min; iPhone and real-device legs not run | `research/2026-09-11_multi-surface_SPIKE-5-6-mobile-crypto-and-background.md`. uniffi go on Android, no JS Noise review; crate share of cold start is Noise IK 7-44 ms plus a 12-111 ms hello; arm64 APK +1.73 MiB with `--exclude-libs` (+7.13 MiB without). Android 15 destroys the socket 6.0-6.4 s after background or screen off, so **push moves to M1+1** and M1 is foreground-only; backgrounded local banners fire 48 s, 164 s, 366 s late at 1, 5, 30 min. M1 entry condition: the same harness on a real iPhone and Android device |
...
| F | `2026-09-13-p1-ainb-app.md` then `2026-09-13-p2-p5-effects-and-hosts.md` | claude-hetzner | worktree `p1-ainb-app`; **merged** #973, #975, #982 (P1 complete); now P2 on `stevengonsalvez/p2-effects` | `term_fa0949da-d441-4575-b345-a22d9386a74a` |
