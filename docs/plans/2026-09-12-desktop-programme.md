...
| slice-1 open gates | 1 | **in review** | | core suite green, fixture findings fixed, human G6 two-terminal check, `v2` CI green | PR #933: `Build site` and `Test ainb installations` green; `tripwire_burndown_heatmap` green on a strengthened selected-day assertion (the arrow key never moved the cursor in any build, `ui.data` was never synced on the Activity arms); burndown Esc fixture findings fixed (`tmux_` prefix, `AINB_HOME`, the `burndown-origin` row asserted); `real_tui_presence_*` skips on a missing artifact instead of reddening `Test`. Remaining red in `Test` is `attention_ingest` x2 plus `fleet::terminal_model_event_can_complete_a_partial_hook_pair`, which are red on `main` HEAD too and belong to the status-T0 lane (#925 comment). G6 steps handed to Stevie for wave 1, S-C and Phase 3: `docs/plans/2026-09-12-g6-human-checkpoint.md` now carries all ten, awaiting his reply |
...
| S-D concurrency tests (wave 4) | 1 | **ready to start** | S-A, S-B, S-C, Phase 2 (all met 2026-09-13) | answer race, resize during answer, surface combo | `tests/answer_race.rs` exists from W0-wire (daemon side); `surface-combo-smoke.sh` absent; plan `:520` |
...
| #916 pane binding without launcher env | 4 | **in review, round 2 pushed** (PR #934) | none | `env -i` hook test, no duplicate legacy row | candidate query fixed: `provider_session_id IS NULL` stops one agent binding another agent's pane, with a test that reds without it. Fingerprint invalidation NOT done and flagged: it needs the stored-identity columns, so it travels with the D14 column set rather than half-landing against derived ones |
| T0-daemon: status store on `fleet_session`, one txn per event, retention, normalizers, OSC schema | 4 | **in review, round 2 pushed** (PR #934) | spike 1 (done), #916, W0-wire migration 0097 (met) | identical tuples across CLI, web, TUI; silence never `done` | both criticals and all nine majors addressed. Migration `0098` unfreezes the tier-5 rows the old authoritative scan stamped; the cross-surface gate now drives the real CLI path; `fleet.status.read` capability added. Two defects found underneath: a replayed event was re-running its projection and closing the card it had raised, and the rollback was not uniform across surfaces. The stored tier/provenance/clocks/incarnation column set is still deferred, with fingerprint invalidation, to one follow-up |
...
|---|---|---|---|---|
| A | `2026-09-12-p0-closure.md`: slice-1 gates green (#925), waves 2-4, G6 handed over | claude-hetzner | **merged** #933, #936, #942, #945, #947, #950, #952, #956 (Phase 2); S-D (wave 4) next, then G6 handover | `term_f52c61ee-95c2-4878-beb8-aaf8b8de6abd` |
...
| C | `2026-09-12-status-t0.md`: #916 pane binding, then T0-daemon, plus the three red daemon tests from #925 | claude-gcp | `feat/916-pane-binding`, PR #934 round 2 pushed (2026-09-12): both criticals and all nine majors from the send-back addressed, `v2` merged, migration `0098` landed. Two items deferred on purpose and named in the PR: stored tier/provenance/clocks/incarnation, and pane fingerprint invalidation, which belongs with them | `term_49ad9b50-e253-497d-ad2c-62059f021b16` |
