...
**Version:** 1.5 (1.1 amended 2026-09-11 after a distinguished-engineer critique, verdict CAUTION 8/10, 30 amendments folded, see `research/2026-09-11_multi-surface_CRITIQUE-amendments.md`; 1.2 folds spikes 1 and 7; 1.3 folds spikes 4, 8, 9 and the provider daemon-reuse hazard; 1.4 folds spike 3; 1.5 folds spike 2)
...
| 2 | **Done 2026-09-12** (`research/2026-09-11_multi-surface_SPIKE-2-control-mode-fidelity.md`, PR #931). The byte stream reassembled from `%extended-output` is byte-identical to a direct `portable-pty` on 8 fixtures at 120x40 and 40x20; 64 of 64 snapshot comparisons across 4 emulator configs match with no normalisation. Crate: `wezterm-term` at `unicode_version: 14` (the only config matching tmux cell advance on all 13 probe glyphs; 413 KB per emulator vs 3.1 MB alacritty_terminal, 4.0 MB vt100). 50 MB flood lossless to the byte. Sizing: a control client is excluded from window sizing only after `refresh-client -C`; under default `window-size latest` it resized a second client from 100x29 to 120x40 | done | R2 | R2 proceeds on the tmux hybrid; emulator is `wezterm-term`; re-seed from `capture-pane` on `%continue` (40 of 40 rows wrong without it; initial seed measured 2 rows) |
...
| 5 | **done 2026-09-14 on the iOS simulator and Android emulators, no real phone** (`research/2026-09-11_multi-surface_SPIKE-5-6-mobile-crypto-and-background.md`; iPhone paired but unreachable, no Android phone attached). A uniffi 0.31 crate over `snow` IK and the real proto types ran in Expo SDK 57 release builds on the iOS simulator (iPhone 17, iOS 26.4.1), an Android 15 x86_64 emulator and an Android 14 arm64 emulator: Noise IK, `auth/hello` and `fleet/subscribe` under AEAD, 15 s heartbeat. Cold start, launch to `auth/hello`: iOS `[simulator]` 1.05-1.68 s (1.17-9.72 s under host memory pressure), Android 15 `[emulator]` 3.9-5.9 s, Android 14 `[emulator]` 1.26-1.29 s; the crate's share is Noise IK 0.9-44 ms plus a 0-111 ms hello (Android 15's first launch after install took 176.3 ms of Noise IK, outside that cold-start range). Size, real release builds: iOS `.app` **+2.45 MiB** (+0.82 MiB zipped `.ipa`); arm64 APK +7.13 MiB at ubrn's default link, **+1.73 MiB** with `--exclude-libs` and `--gc-sections`. Custody via `expo-secure-store`: Android Keystore survives an update, not an uninstall, both emulators; iOS Keychain `AfterFirstUnlockThisDeviceOnly` survived uninstall and did not enforce `requireAuthentication` on the simulator, so both iOS custody answers are unmeasured | 2 days | M1, `ainb-wire-mobile` | uniffi is the M1 wire shape; no JS Noise review; M1's entry gate runs this harness unchanged on a real iPhone over USB and a real Android 15+ phone, records iOS keychain survival and Face ID gating, and flips this only if the device slice fails in the crate; ship the symbol-hiding link flags on Android |
| 6 | **done 2026-09-14 on the iOS simulator and Android emulators, no real phone** (same report). iOS `[simulator]`: no heartbeat later than 1.5 s after Home or lock in 11 of 11 runs (first missed beat 3.6-16.5 s; JS ran up to 4.2 s, then the app read as suspended); the socket stays open while suspended, none reset in 600 s in 6 runs, reset at 264-424 s in 5 (2 verified as memory kills); on foreground the tokio heartbeat bursts every missed tick, and one undisturbed run resumed its 15 s cadence. Android 15 `[emulator]`: socket destroyed **6.0-6.4 s** after HOME or screen off in 9 of 10 runs, the tenth lost every beat without a reset (`blocked=APP_BACKGROUND`). Android 14 `[emulator]` on AC with stay-awake: socket and heartbeat alive for 600 s in 10 of 10. Android banner lateness is for `SCHEDULE_EXACT_ALARM` not granted; the granted path was not measured. Local banners with the app locked: iOS `[simulator]` 1 min +1 s, 5 min +1 s, 30 min +7 s; Android 15 +48 s, +164 s, +366 s; Android 14 +47 s, +226 s, +364 s (inexact alarms, windows of 75 percent of the delay). iOS 26.4.1, Low Power Mode not available on the simulator; Android battery saver off | 1 day | M1 gate wording | socket survival, the threshold's measure: Android 15 6 s, under 60 s; iOS simulator 600 s in 6 of 11 runs and 264-424 s in 5, over 60 s but suspended and silent (heartbeat none past 1.5 s), so it cannot raise a banner: **the push reopen row moves to M1+1**, carried by Android 15; M1's socket is foreground-only, reconnects with `after_revision` on every foreground, its heartbeat skips missed ticks, and it gates no banner timing while backgrounded |
