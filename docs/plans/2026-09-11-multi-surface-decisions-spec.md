...
| 5 | **Android done 2026-09-14 on an emulator, iOS blocked** (`research/2026-09-11_multi-surface_SPIKE-5-6-mobile-crypto-and-background.md`; no phone attached, no Xcode on the box). A uniffi 0.31 crate over `snow` IK and the real proto types ran in an Expo SDK 57 release build on an Android 15 emulator: Noise IK, `auth/hello` and `fleet/subscribe` under AEAD, 15 s heartbeat. Cold start, 5 runs: launch to `auth/hello` 3.9-5.9 s, of which the crate's share is Noise IK 7-44 ms plus a 12-111 ms hello; the 0.7-1.0 s WS connect is the emulator NAT. arm64 APK delta: +7.13 MiB at ubrn's default link, **+1.73 MiB** (722 KiB gzip) with `--exclude-libs` and `--gc-sections`, run end to end. Key in Keystore via `expo-secure-store`: survives an update, not an uninstall, not biometric-gated; a gated write fails with no enrolment | 2 days | M1, `ainb-wire-mobile` | uniffi is the M1 wire shape on Android; no JS Noise review; an iPhone and a real Android device run of the same harness becomes M1's entry condition, which flips this if the iPhone leg fails; ship the symbol-hiding link flags |
| 6 | **Android done 2026-09-14 on an emulator, iOS blocked** (same report). Android 15: the OS destroyed the live socket **6.0-6.4 s** after HOME (5 runs) and after screen off (4 of 5; the fifth lost every beat with no reset), with `blocked=APP_BACKGROUND` in netpolicy and the process frozen at +10 s; no heartbeat arrived after the transition in any run. Local banners with the app backgrounded and screen off all fired, all late: 1 min +48 s, 5 min +164 s, 30 min +366 s, inexact alarms with windows of 75 percent of the delay (`SCHEDULE_EXACT_ALARM` not granted by default) | 1 day | M1 gate wording | grace 6 s is under 60 s: **the push reopen row moves to M1+1**; M1's socket is foreground-only, reconnects with `after_revision` on every foreground, and gates no banner timing while backgrounded |
