...
| 6 | **done 2026-09-14 on the iOS simulator and Android emulators, no real phone** (same report). iOS `[simulator]`: no heartbeat later than 1.5 s after Home or lock in 11 of 11 runs (first missed beat 3.6-16.5 s; JS ran up to 4.2 s, then the app read as suspended); the socket stays open while suspended, none reset in 600 s in 6 runs, reset at 264-424 s in 5 (2 verified as memory kills); on foreground the tokio heartbeat bursts every missed tick, and one undisturbed run resumed its 15 s cadence. Android 15 `[emulator]`: socket destroyed **6.0-6.4 s** after HOME or screen off in 9 of 10 runs, the tenth lost every beat without a reset (`blocked=APP_BACKGROUND`). Android 14 `[emulator]` on AC with stay-awake: socket and heartbeat alive for 600 s in 10 of 10. Local banners with the app locked: iOS `[simulator]` 1 min +1 s, 5 min +1 s, 30 min +7 s; Android 15 +48 s, +164 s, +366 s; Android 14 +47 s, +226 s, +364 s (inexact alarms, windows of 75 percent of the delay). iOS 26.4.1, Low Power Mode not available on the simulator; Android battery saver off | 1 day | M1 gate wording | grace is a few seconds on iOS (heartbeat none past 1.5 s) and 6 s on Android 15, both under 60 s: **the push reopen row moves to M1+1**; M1's socket is foreground-only, reconnects with `after_revision` on every foreground, its heartbeat skips missed ticks, and it gates no banner timing while backgrounded |
