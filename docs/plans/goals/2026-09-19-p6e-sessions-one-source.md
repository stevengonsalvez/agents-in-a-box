...
· The daemon's own registration follows the same discipline. On P6d, `run_loop.rs:1900` writes the file through `register_session`, which takes and releases the flock inside, and `:1903` then writes the table with no flock held, so a `Persist::SessionHeadroom` compare-and-set in a surface can interleave with it and lose an update. P6e takes the flock once around both steps (a `register_session_at` variant for a caller that already holds it), file row first, then the table, bounded by `SESSION_RPC_DEADLINE`'s flock rule. A table failure there stays best-effort and logged, as the degrade rule wants for an already-live run; the file row it left is picked up by the next reconcile pass.
