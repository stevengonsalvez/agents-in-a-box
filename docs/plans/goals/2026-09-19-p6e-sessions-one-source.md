...
· The reconcile is repeatable. It runs on every daemon boot, on a surface's move out of the degraded state (see "Daemon down at startup"), and whenever `sessions.json`'s mtime has changed since the last pass, checked at most every 30 s. The boot pass does not hold the RPC socket: the socket opens at once and the pass runs in the background, so a CLI holding `sessions.json.lock` cannot delay startup. Until that first pass finishes, `session_list.import_complete` answers false and no session read is served from the table, so clients stay on the file; one daemon test connects while the lock is held and proves it. Until the flip the file is the authority on which sessions exist, and the table on their contents (amended on #1250, the orchestrator's rule). Each pass first deletes every table row whose session the file does not have, returned on the pass's outcome and in the `workspace/session_reconcile` result as `deleted` and logged one line per row (not a marker column: that would take a migration); then inserts every file session whose id is absent from the table; a row both have keeps its table contents. The deletes close three gaps: a delete that reached the file but not the table (a client that crashed between the two), a delete made while degraded (file only), and a multi-row write whose file change was reverted after some table rows had landed. A tmux-name conflict survives only when the table binds the name to an id the file also has: such a file session is not inserted (P6d `upsert` refuses it) and is counted on the pass's marker as skipped, with one warning naming both ids, never silently dropped. The rule is sound only while every writer writes the file row before the table row. Every pre-flip writer does: import and reconcile read the file, `SessionSource::Daemon` writes the file row first, and the daemon's own registration writes no table row when its file write fails (next bullet).
· A reconcile pass holds the `sessions.json` flock (`lock_sessions_store_at`) from before it reads the file until `complete_import` commits, so no flock-taking writer can add or remove a row between the read and the marker. P6d's import reads the file unlocked (`session_import.rs` `read_capped`); `save` is temp plus rename so the read is never torn, but a row written in that window would be missed. The residual window is a writer that does not take the flock at all; v2 has none (every `sessions.json` writer has taken it since pu4), and the call-site tripwire of criterion 1 keeps it that way.
· The daemon's own registration follows the same discipline. On P6d, `run_loop.rs:1900` writes the file through `register_session`, which takes and releases the flock inside, and `:1903` then writes the table with no flock held, so a `Persist::SessionHeadroom` compare-and-set in a surface can interleave with it and lose an update. P6e takes the flock once around both steps (a `register_session_at` variant for a caller that already holds it), file row first, then the table, bounded by `SESSION_RPC_DEADLINE`'s flock rule. The table row is written ONLY once the file row is (amended on #1250): a failed file write is retried on a short bounded backoff (0.25 s, 1 s, 4 s), and if it still fails nothing is written to the table, the failure is logged, and an error line on the run's live task stream tells the operator the session is running but not in the session list. A table failure after the file row is written stays best-effort and logged, as the degrade rule wants for an already-live run; the file row it left is picked up by the next reconcile pass.
· Old-binary deletes are reconciled too (amended on #1250, following the existence rule above): a previous release's `ainb kill` removes the file row only, and the next pass deletes the table row, so the new stack stops listing that session within one reconcile interval. The one thing no pass does is read a missing `sessions.json` as "no sessions" while the table holds rows: that pass is refused and the rows kept, since a file that went away is far likelier than a user who killed everything. A fresh home (no file, no rows) still completes its pass. A test pins each.
...
· No resurrection. The reconciliation never re-inserts a session the table deleted, and never overwrites a table row with an older file row. Before the flip the file decides existence (a table row the file lacks is deleted), so a delete is complete once it reaches the file.
...
1. One source for every surface, proved at runtime. Daemon state: a daemon that advertises the capability with import and reconcile complete, one private hangar home. A TUI (the `ainb-app` host under test), a desktop host and a CLI each report the `SessionSource` variant they resolved (a debug accessor on `session_source`, read by the test); all three answer `Daemon`, and a session each one writes is visible through the other two in that same run. A second run starts the TUI with no daemon so it resolves `Degraded`, then starts the daemon and runs a CLI that resolves `Daemon` beside it: the CLI's session reaches the TUI and the TUI's degraded-time session reaches the CLI once the TUI has switched (criterion 8), and before the switch the TUI shows the degraded notice rather than a silently different list. Static support: a call-site tripwire in the shape of `tests/serialize_guard.rs`, with a committed fixture, fails when a new direct `SessionStore::load`, `mutate` or `lock` call appears outside the `File` and `Degraded` implementations and `SessionStore` itself; it backs the runtime test, it does not replace it.

2. The TUI sees what the CLI writes and the reverse. Daemon state: a real daemon in a private hangar home, capability on (the test switch before the flip), import and reconcile complete; both processes resolved `Daemon`: a session created by `ainb run` appears in the TUI's workspace list read (`state.rs:3110` path) without a restart; a session the TUI creates (`session_manager.rs:1575` path) appears in `ainb list --format json`; `ainb kill` of a TUI-created session removes it from the TUI's next read; `Persist::SessionHeadroom` keeps its compare-and-set semantics through the daemon (a test where the expected value moved leaves the row unchanged), including against a concurrent daemon registration of another session, which must not lose the headroom update (red with the registration's table write moved back outside the flock). Each is an integration test that fails when the corresponding site is reverted to the file.

3. Reconciliation without resurrection, repeatable. Daemon state: capability on (test switch before the flip), P6d import row present. Each pass (boot, degraded exit, mtime change) inserts every file session whose id is absent from the table; a table row is never overwritten by its file row; a session deleted through the new stack is gone from the file too, so a later pass does not bring it back; a table row whose session the file no longer has is deleted by the next pass (amended on #1250: a file-only delete after a crash, a degraded delete, a reverted multi-row write, one test each); a session an older binary appended to the file after the previous pass is inserted by the next one; a file session whose tmux name the table binds to another id the file also has is skipped and counted, and the table row is unchanged (table wins on contents); a failed pass writes no marker and keeps `session_list.import_complete` false even though the P6d import row exists (the kind-aware check of open question 3). Six tests, one per clause, each red when its guard is removed, plus one that a writer blocked on the flock during a pass lands its row in the file after the pass and is inserted by the next pass, never lost. No tombstones: see open question 1.
...
2. **Does a session created after the flip survive a downgrade?** Decided: yes. The file stays written, row by row under its flock, with the table ("Mixed versions"). Writes from an older binary are picked up by the repeatable reconcile; since the existence rule (amended on #1250) so are deletes from an older binary, tested by criterion 7b. Removing the file write is a later node with its own spec note.
