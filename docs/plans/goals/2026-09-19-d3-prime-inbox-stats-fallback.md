...
The second seam is the one the inbox cannot skip. The daemon's ledger guard only engages when the caller sent an op id (`rpc/mod.rs:1341-1342`); without one, a retry after a lost reply runs the sweep again, which is harmless for this method (it is idempotent) and is exactly the habit that is not harmless for the next one. So the op id is minted once per effect by the executor that sends it, the retry sends the same id and the same body, and a test proves the second send is answered from the ledger (`replayed`), not by the handler. Two things about the verb are said plainly rather than implied. It is a whole-inbox sweep: `handle_inbox_mark_read` calls `InboxRepo::mark_all_read` (`rpc/snapshots.rs:2084-2090`), there is no per-entry verb (`methods.rs:1174-1210`), so every control on every surface is labelled "mark all read", never "mark read" on a row. And its fence is `Fk::None` (`mutation.rs:1190`), so a client holding a stale list marks rows it never drew; the goal records that and does not invent a fence, since the registry is the committed artifact. Two surfaces sweeping is two op ids and two sweeps: the second answers `marked: 0` from the handler, which is not the `replayed` path, and the test names both cases so nobody reads `marked: 0` as a ledger hit. The sixth seam is the cheap one: an older webview or TUI reading `inbox` sees keys it did not have, and serde's default for unknown keys is to ignore them; the PR says so and a test shows an `InboxView` of D3a's shape decoding a D3p-a frame.

─ SCOPE, STAGED AS PRs ─

Seven PRs, the inbox first as four of its own, then the amendment for the other two, then the stats tab, then the fallback cell last and separable, since it is the one the node could ship without. Each is mergeable alone, targets `v2`, and carries its own proof. The inbox needs no amendment: `spec:223` is its contract already. The stats tab and the fallback cell each rest on a claim the spec does not yet make, so their amendment goes before them, as D3·spec did.
...
· Builds: `InboxSection { entries: Vec<InboxEntryRow>, unread: i64, recipient: String, absent: Option<String>, head_revision: i64, rows_cut: usize, summaries_cut: usize }`. The bound is bytes, not a row count: the daemon already caps `inbox_list` at 200 rows (`ainb-hangar-daemon/src/rpc/snapshots.rs:1888`, `INBOX_LIST_LIMIT`, applied at `:1908`), so a row cap at 200 would never fire, and what is unbounded is `summary`, which the aggregator writes as `format!("New issue: {}", row.title)` (`inbox_aggregator.rs:84`, `:90`) with no cap on the title anywhere. So the fold keeps `MAX_INBOX_ROWS` at 100, below the daemon's cap so the cut path runs on a real daemon and `rows_cut` is not zero by construction; truncates each `summary` at `MAX_SUMMARY_CHARS` (256) with a marker, counted in `summaries_cut`; and proves the section's encoded bytes stay under a budget of its own, well inside `MAX_FRAME_BYTES` (`frame.rs:149`), because an over-size section is withheld WHOLE into `FrameBatch.oversize` (`frame.rs:169`) and a withheld inbox is a blank inbox with no `rows_cut` to explain it. The projection is the D3a shape: scrub before cut, never after. `InboxView` frames each field with its scrubber or its allow-list reason (`summary` through `scrub_str`, `fields.rs:168`, then the cut; ids, enums and timestamps allow-listed with the reason in the triage); the reader, one read in flight, a subscription-shaped lifecycle if #1215's is on `v2`, else a poll on the tick cadence `agent_status` uses, folded on the host tick; the mark-read effect carrying a minted `OpId`, sent as `InboxScopedParams { workspace_id: "default", recipient: Some("member:me"), mutation }`, its reply folded into `unread` through the same drain as a read. The reducer flips nothing optimistically: `unread` is the daemon's number or `absent`.
· Proof: `ainb-app` tests that 200 rows with 64 KiB summaries frame inside the section's byte budget and say what was cut in both counters, with the encoded byte count asserted, not inferred from the row count; that a credential-shaped `summary` assembled at runtime and placed past the 256-character cut does not survive the frame while the redaction marker draws; that a `MutationEnvelope::default()` cannot reach the wire from the effect (the type the executor sends has no default constructor for the op id); a daemon-side test in `ainb-hangar-daemon/tests/rpc_inbox.rs` that two sends of the same op id and body run the sweep once and the second is `replayed`; the key-path fixture regenerated with `UPDATE_SECTION_KEY_PATHS=1` in the same commit as each field, after triage; a test that an `InboxView` of the previous shape decodes the new frame (seam 6).
...
· Builds: the screen over `state.inbox`: rows newest-first with kind, age and summary, unread rows marked, the unread badge on the home hint, `r` bound to "mark all read" (the verb sweeps the whole inbox, so no row-level control is drawn), `Enter` a no-op until a deep link exists (the row carries `subject_id`, and where it goes is a follow-up, not a stub). Registered from `layout.rs` after `:313`, so `ainb-core/src/app/*` is not touched.
...
· Builds: the inbox as a surface beside the board, rows from the frame, unread count as a scalar root selector in the header, a "mark all read" control that sends the `RendererIntent` and nothing else (no per-row control, since the verb has none); the `absent` reason drawn when the daemon has no inbox; the `rows_cut` and `summaries_cut` counts drawn when a bound was hit, and a truncated summary drawn with its marker.
...
· Builds: one `inbox` fixture with 101 rows, so `rows_cut` is one and drawn as one, one of them read, one summary past the 256-character cut so `summaries_cut` is one and the marker draws, drawn by both renderers against one expected-facts list; the mutation check re-proved on this fixture (one fact deleted from one renderer's output, the suite fails, the failing output on the PR); the wdio journey: the inbox opens, the rows drawn carry the daemon's row ids, "mark all read" sends one op id and the unread count reaches zero from the daemon's reply, not from the webview; the proof scenario writing `result.json` with `pass: true` and observed lines naming the sections applied, the rows drawn, the op id sent and the ledger row it left.
...
· Builds: the runtime brought up on the first `plugin.owned.watch_screen` for a screen in the desktop's watchable set, which is `PLUGIN_SCREENS` (`ainb-app/src/app/screens/builtin.rs:15-21`) MINUS `analytics`: the stats tab already shows burndown's counters from the daemon's projection, and a cell blitting burndown's paint beside it would put the same numbers on the desktop twice from the two scans this node itself files as drift. So the watchable set is `witr`, `learnings`, `abtop` and `hangar`, a watch for `analytics` is refused at the intent seam with that reason, and a test pins the set; renewed on the lease the section already keeps, stopped with the window; cells drained with `try_recv_render` on the shell's tick and sent on a `Channel` as the terminal's bytes are, painted in the webview into a cell grid (the existing xterm.js surface fed the buffer as escape sequences, if a `ui/` test shows it paints the TUI's placeholder text identically; else a plain grid, decided on the PR); the three placeholders from `PluginsHostView` when there is no runtime, no plugin, or no frame yet; keys and clicks on the cell sent as `plugin.owned.action` through the host-neutral routing.
...
2. **Mark all read is a D18 mutation end to end, and is named as the sweep it is.** Every control on both surfaces reads "mark all read"; the effect carries a client-minted op id; a retry sends the same op id and body and a daemon test proves the second send is answered `replayed` from the ledger with the sweep run once; a second test sends two op ids and proves two sweeps, the second answering `marked: 0` from the handler; the PR body records that with `Fk::None` a stale client marks rows it never drew; and no `MutationEnvelope::default()` is added anywhere in this node (`git grep` count on the PR equals the count on `v2`).
...
7. **The fallback cell paints a runtime the shell owns.** A plugin screen in the watchable set (`PLUGIN_SCREENS` minus `analytics`, pinned by a test) watched from the window renders live cells over a channel that is not a section, a watch for `analytics` is refused with its reason, the three placeholders draw without a runtime, `ainb-app` still owns no runtime, and `plugins_host` is a fixture on both halves, raising the floor to sixteen.
...
· How many inbox rows a live daemon returns for the default workspace, the bytes the section framed, and how often the 100-row and 256-character cuts were hit.
...
· A per-entry mark-read verb on the daemon, so a surface can mark the row it drew rather than the whole inbox; today `hangar/inbox_mark_read` is `InboxRepo::mark_all_read` (`rpc/snapshots.rs:2090`) and nothing else exists.
