...
· Builds: the inbox as a surface beside the board, rows from the frame, unread count as a scalar root selector in the header, a mark-read control that sends the `RendererIntent` and nothing else; the `absent` reason drawn when the daemon has no inbox; the `rows_cut` and `summaries_cut` counts drawn when a bound was hit, and a truncated summary drawn with its marker.
