...
- Named deferrals: the daemon attention poller and live-window watcher are not started by the desktop host yet, so Fleet frames carry no daemon rows and rings show only local errors; the settings entry is drawn inert (D3); the tab strip is drawn empty with the new tab control inert (D1c); the palette is D1d.
