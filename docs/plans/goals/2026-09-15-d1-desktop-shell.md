...
- Credit window, `WINDOW_BYTES` = 4 MiB: the pump sends only while the webview has acknowledged (after `xterm.write` paints) all but 4 MiB of what it was sent. Reason: a slow or hidden webview then stalls the PTY reader and tmux holds the output, so memory in flight per tab is bounded by 4 MiB, while a burst still paints without a round trip per chunk. Reads coalesce up to 64 KiB per message.
- Tab cap, `MAX_ATTACHED_TABS` = 8 per host, from base spec `:250`: opening a ninth detaches the tab idle longest (least recent input or painted output), a toast names it, the tab stays listed and a click re-attaches. Reason: each attached tab is a tmux client plus four threads (reader, pump, writer, reaper), so the cap bounds both.
...
- Measured: 50 MiB through the PTY and the pump, no tmux, in about 0.37 s with every byte delivered (`terminal::tests`, recorded, not gated).
