...
- Deferred: the wdio sidebar leg (D1d); the root-selector scalar check, with `ROOT_SELECTORS` exported (D1c); `evictHost` and an active-host signal (D2); one `Mirror` per subscriber instead of the single-slot `ChannelSink` (the node that opens a second window); the `WorkspaceLoad` subscription (D1c). Filed as issues: the merged attention on the wire (`Session::live_attention` and `provider_session_id` are not serialised, so the renderer derives a weaker merge; D1c), a `framesIgnored` counter, and the `reconcile` divergence from the fan-out bench.
