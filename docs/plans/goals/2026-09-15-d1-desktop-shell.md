...
- Subscription answer: Sessions, Shell, Tmux, Fleet, Config and `AgentStatus`, as recommended (`main.rs` `SECTIONS`, `main.tsx` `SUBSCRIBED`).
...
- Named deferrals: the daemon attention poller starts inside `AppState::update_tmux_previews`, which the desktop host does not call yet, so Fleet frames carry no daemon rows and rings show only local errors; the settings entry is drawn inert (D3); the tab strip is drawn empty with the new tab control inert (D1c); the palette is D1d.
