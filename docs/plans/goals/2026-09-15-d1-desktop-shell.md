...
- Deferred: the wdio terminal leg and the 50 MiB number through the webview (D1d, with the wdio harness); the palette, attention jump and host switcher accelerators are recognised but do nothing (D1d, D2, R1); open tabs are not persisted to `desktop.json`; `keymap.toml` chords are not checked against terminal focus at load, since the desktop has no TOML accelerator table yet; in-place preview, observer, tool, workspace shell and login targets; #1131 (attention on the wire) is its own PR after this one; macOS Cmd+W collides with Tauri's default Close Window menu item (D1d builds a menu without it); transport-side tests; ctrl+shift+c and ctrl+shift+v copy and paste on Linux; a desktop context on the command gate instead of the second synthetic navigation `open_sessions` does (D3); the tab registry as a reducer section and `WINDOW_BYTES` per transport leg (D2); collapsing the pump thread into the reader once the registry moves to the reducer. Dependency notes: the lock carries nix 0.27.1, 0.28.0 and 0.29.0, and winreg arrives transitively (0.10.1, 0.50.0, 0.55.0), the review flagging the unmaintained one.
