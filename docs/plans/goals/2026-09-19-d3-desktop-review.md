...
· Splits the programme row rather than dropping any of it. `docs/plans/2026-09-12-desktop-programme.md:134` reads "D3 review, inbox, settings, burndown, fallback cell | 3 | planned | P5 | full parity suite". It becomes two rows: **D3 review and settings**, gate "parity suite for the screens D3 draws, with the mutation check"; and **D3-prime inbox, burndown stats, fallback cell**, depending on D3, gate "full parity suite". D4-prime then depends on D3-prime rather than on D3. Nothing leaves the programme: every item in the old row is in one of the two.
...
─ WHAT D3 DEFERS, WITH ITS COST ─

Neither of these is dropped: each moves to D3-prime with what it would take.
· **The stats tab.** Burndown publishes no `ui.state` at all, only `ui.close_request` (`ainb-plugin-burndown/src/plugin.rs:339-350`), and lives off the session topics it subscribes to (`:153-165`). So a stats tab is not a component over a section that exists: it is a new daemon read for counters no section carries, which is the same read the D2 amendment deferred for the board's stat strip (`spec:214` and the amendment above it). Landing it inside D3 would put a second projection of usage beside `agent_status`, which is what D14 exists to stop. Cost to record: the daemon verb, the rows, their bound, and whether the numbers are a section or a read the screen makes.
· **The plugin fallback cell.** The desktop runs no plugin runtime (`ainb-desktop/src/executor.rs:29`), so a cell means standing up a plugin host in the window with the quarantine rules the TUI's has, and the paint path the TUI uses (`ainb-core/src/app/screens/builtin.rs:422-462`) blits cells a runtime produced. Cost to record: the host, its lifecycle, and whether the cell reuses the PTY path D1c landed or a second channel.
...
1. **The parity suite passes in both halves, and it can fail.** Every screen D3 draws has a committed fixture, rendered by the ratatui backend and by the webview on a node runner over the same fixture, and diffed against one expected-facts list. The fixture floor rises from twelve (`ainb-app/tests/parity.rs`) to fourteen, and the suite carries its own mutation check: with one expected fact deleted from one renderer's output, the suite fails. A green run with a stubbed DOM half is a failed criterion. The review tab's fixture is named for the tab, not `code_review`, because `git_view.json` already claims that screen id and two fixtures naming one screen is how a suite silently drops one.
2. **The wdio review journey is green in CI**, on `ubuntu-latest` under `xvfb-run` and on `macos-latest` with the leg `MACOS_E2E_LEG` declares, against a repository whose diff is over a recorded byte floor rather than a toy: the floor is on the PR, and it is large enough that the bound in criterion 4 is exercised. In one run: the review tab opens, the rows drawn carry the reducer's own row ids, a selection sent from the window lands on the row the frame named, and the settings page reads the config section. The claim "the hunks are the reducer's model, not a diff computed in the webview" is proved by a `ui/` test that every row id drawn came from the frame and none was minted in TypeScript, not by the journey.
3. **The proof harness scenario `d3-review` passes on a `v2` build**, writing a `result.json` with `pass: true`, whose observed lines carry the sections the renderer applied, the review rows drawn, the settings page's reading of the config section, and the daemons panel's reading of each daemon. It is in `ALL_NODES`, and a box with no headless X server records a `skip` rather than a failure.
4. **The `git_view` section is bounded, and what it cut is visible.** A repository diff past the bound frames inside `MAX_FRAME_BYTES` (`wire/frame.rs:149`) with a row window and a per-file cap, an `ainb-app` test proves it, and the frame says what was cut rather than reading as a short diff. The webview draws `FrameBatch.oversize` (`frame.rs:169`): the section name and the reason, on the review tab first, with a `ui/` test. How often the bound was hit on a real repository is recorded on the PR.
5. **Every field D3 added or changed on the wire is provably scrubbed and provably in sync**: `section_key_paths.txt` names each new leaf, regenerated in the commit that added it; the diff text keeps `scrub_rows` (`ainb-app/src/components/code_review/model.rs:75-80`) through the bound, and a test proves a credential-shaped string assembled at runtime does not survive a hunk while the redaction marker still draws; `bindings/AppState.ts` is fresh and `tsc --noEmit --strict` is clean.
6. **#1161, #1162 and #1175 are closed by this node**, each with its evidence: the palette's rows come from the reducer for every surface, diffed against the golden row set captured from the host in the commit before the move, so the test cannot pass by comparing the new code to itself; the spec says which throughput number each path can prove; and the onboarding writes have a desktop path the webview cannot script, with the refusal toast pointing at it.
7. Final deliverable runs without errors, with the test output and the CI run ids on the PR.

The softer ones, measured and recorded rather than gated:
· The bytes a real repository's diff framed at the bound, and how many times the bound was hit in a session.
· What the stats tab's counters would cost as a daemon read, for D3-prime.
· The number of sections a review-and-settings drain applies, from `renderer_applied`, so D3-prime and D4-prime know what the last surfaces cost.
...
· No renderer in this repository reads `FrameBatch.oversize` (`wire/frame.rs:169`). D3b draws it on the review tab; every other screen still goes quiet when its section is withheld, which is a surface-wide gap worth its own issue.
· There is no rollback for a renumbered section set: `SectionId` order is what a surface subscribes by, and D3-prime adds fields to a section that has been empty since the extraction. File the question of what a surface does when it meets a section set it does not know.

─ OPEN QUESTIONS THE LANE ANSWERS ON A PR BODY ─

· **Where the inbox's rows come from.** Answered, and carried to D3-prime: `hangar/inbox_list` (`ainb-hangar-proto/src/methods.rs:1173`), read by the host the way `agent_status` reads `fleet/roster_status`, with `hangar/inbox_mark_read` (`:1192`) as the only write and an envelope on it per D18. The spec's inventory still says "list from notifyd SQLite via core section" (`:199`), which is stale: the daemon aggregates the inbox itself (`ainb-hangar-daemon/src/lib.rs:147`), a second process opening notifyd's database would be a third reader of one store, and the rows already have a wire shape in `InboxEntryRow` (`ainb-hangar-proto/src/events.rs:1061-1086`). D3·spec records it; D3-prime builds it.
· **Whether the inbox screen returns to the TUI or only to the desktop.** Answered: both, and neither in this node. The section is the boundary and a screen on one surface only is the drift the section set exists to stop, so D3-prime lands the pair together.
· **How a plugin view reaches the desktop.** Deferred with the stats tab and the fallback cell; the recommendation stands for D3-prime, not for this node. Recommended: the fallback cell paints the plugin's own cells, the way `PluginScreen` already blits them in the TUI (`ainb-core/src/app/screens/builtin.rs:422-462`), and the window acts on that screen through `plugin.owned.watch_screen` and `plugin.owned.action` (`ainb-app/src/app/plugin_action.rs:15-33`), so no plugin JSON rides a frame. Burndown makes the stats half easy to scope: it publishes no `ui.state` at all (`ainb-plugin-burndown/src/plugin.rs:339-350`) and lives off the session topics at `:153-165`, so a stats tab is a component over that data rather than a render of the plugin's paint. If the lane wants `ui.state` on the wire instead, that is an amendment carrying a proof that a plugin view can be shown free of secrets, not a field. Answer on D3·spec.
· **Whether the review tab brings a text editor dependency.** Answered: no. The rows arrive already split by the reducer (`ainb-app/src/components/code_review/model.rs`), so the tab paints a styled list of rows it is given, and an editor joins only when highlighting or folding is shown to fall short, as its own decision with a package, a version and a licence on the PR.
· **What the DOM half of parity renders with.** Answered: the existing node test runner over the same fixtures, rendering the Solid components headless and comparing one expected-facts list, rather than a second browser run, because the wdio journeys already cover the real window. It ships with the mutation check in criterion 1; without that, a stubbed renderer passes.
...
D3d, lane T, branch `d3d-settings` cut from `d3a-seams` (2026-09-19):
- The settings form writes through one new pointer row, `config.set_row {key, value}` (`ainb-app/src/app/pointer.rs`), which resolves the edit against the row's own kind and then takes the popup's path: `set_row_value` and `persist_config_screen`, so the write is `Persist::AppConfig { keys }` naming the edited key. The popup's confirm now shares that helper (`apply_config_row_edit` in `events.rs`). No `AppConfig::save` from the window, and no new frame field: the `config` section already carried the screen state through the redaction seam, so `section_key_paths.txt` is untouched.
- The row edit is a `ConfigRowEdit` (`config/settings_model.rs`): text, a secret's reference, a bool, a choice index, a number. A plain text edit never lands on a secret row, an index past the options is dropped, and `[usage]` rows stay read-only. Pinned by `tests/config_row_command.rs`.
- Opening the page walks the reducer onto the Config screen through `global.go_home` and `home.config`; closing it goes `config.back` then `home.sessions`. Pointer rows on the config screen run only there, which is the context gate doing its job rather than a rule added for the desktop.
- The daemons panel draws `hangar.daemons_state` (subscribed as `hangar`). The terminal arms and touches the collector from its paint; the desktop does the same from the host tick while the reducer is on Config (`ainb-desktop/src/daemons_panel.rs`), bumping the section only when a collection landed.
- #1175: the native path is `ainb-desktop/src/setup.rs`, called by the Tauri command `setup_write` behind a native message dialog (`tauri-plugin-dialog` 2.7, Rust side only, no capability for the webview). The write calls the same `ainb-app` functions the wizard calls (`install_dep_capture`, `install_tmux_config`, the `otel` writers), outside the reducer, so `Keymap::is_key_only` is not relaxed and `key_only_completeness.rs` gains no row. The refusal of an onboarding write from the window now reads `it writes outside ainb; the window runs it from Settings, Setup, behind the shell's own confirmation` (`setup::desktop_path`). Pinned by `ainb-desktop/tests/setup.rs`.
- Parity: `config.facts` and `daemons.facts` beside the fixtures, read by both halves. The ratatui half checks the snapshot in `ainb-core/tests/parity_snapshots.rs`; the DOM half is `ainb-desktop/ui/src/parity.test.ts`, rendering `settingsLines` from `tests/parity/frames/<fixture>.json`. Each half carries the mutation check: one fact deleted from the render, the check names it. The node runner strips types but not JSX, so the DOM half renders the projection `settings.tsx` prints one to one rather than the component; a Solid SSR render is the upgrade path if the orchestrator wants literal DOM text.
- No new fixture: the settings page is the `config` and `daemons` screens, which already have fixtures, and a second fixture naming `config` is the drop the goal warns about. The floor stays where D3b leaves it; asked on the PR.
- Not in this branch: the wdio review journey, the `d3-review` proof scenario, the CI wiring and the programme row. The goal folds them into the last D3 PR; which lane lands them is asked on the PR.
- Deferred with a note: `hangar_daemon.*` rows edited from the desktop queue on `pending_daemon_config_edits`, which only the terminal host drains (`ainb-core/src/main.rs`), so a daemon-row edit from the window waits for a terminal to flush it. The form saves them like any other row; the gap is the desktop host's missing async drain, filed as a follow-up.
- Security review of D3d (#1224), fixed on the same branch: a renderer's row edit is judged twice, against a deny list of rows whose value reaches a program the host runs (`ainb-app/src/config/renderer_edit.rs`, with the spawn each reaches) and then an allow list of the rows the page draws; a row on neither is refused too. The judgement runs in `AppState::remote_command_refusal` for `config.set_row` by name and for the key sequence (Enter on the row, Enter in its popup), and in `resolve_config_row_edit` for the payload; the reducer's command gate and the desktop's seam now judge a command with its payload parsed, not the row's placeholder. `tests/config_renderer_edits.rs` enumerates every spawn-reaching row and proves each refused both ways; `tests/fixtures/renderer_editable_rows.txt` commits the verdict per registry row, and `ui/src/parity.test.ts` diffs the page's copy of the policy against it. The telemetry token is redacted from `SetupWrite`'s Debug and the log line carries the variant name only; the dialog names the endpoint and instance id and requires https; a text row edits from empty with its scrubbed value as the placeholder, and the marker is never written back; one setup dialog at a time.
- Security review of #1243, fixed on the branch: the policy in `renderer_edit.rs` is two exact lists of registry keys, deny by default, no prefixes; `acp.adapters.*.permission_mode`, every `fleet.bridge.*` row, the scan paths, `web.read_only` and container ports are denied; `tests/config_renderer_edits.rs` walks `registry::rows()` so a new row fails until classified, and checks every executable-shaped leaf is denied. Secret rows are not renderer-settable in D3d: `ConfigRowEdit::Secret` is gone from the wire, the page shows presence and source only, and the popup, `ctrl+k` keychain prompt and API key prompt are refused from a renderer with `SECRET_REASON`. `DesktopHost::refused_from_renderer` fails closed on a payload the row cannot parse. A text edit is cleaned of control and format characters and refused over `MAX_TEXT_CHARS`. From the code review, landed with these: a blank number input is not an edit of 0; `config.set_row` carries the frame's revision and a stale one is refused with a notice; a dropped edit says why; `config.select_node {id}` gives the reducer the tree selection the page used to keep.
- Code review of #1243, fixed on the branch after the security items: the page keeps no state of its own (whether it is open is `shell.current_screen`; the tree selection is `config.select_node {id}`; the rows drawn are `visible_nodes` and `visible_rows`, so the `/` filter and the tree expansion are the reducer's and the search box types into the reducer's own search); the DOM half of parity mounts `SettingsPage` through Solid's server renderer (`ui/test/solid-ssr.mjs`, `npm run test:parity`), and its mutation check takes a daemon row out of the frames before rendering, as the ratatui half's takes a file out of the state; the `daemons` fixture seeds three collected rows so its facts are data; the facts lists moved to D3b's `tests/parity/facts/<fixture>.txt`, read by `parity.rs`, `parity_snapshots.rs` and the DOM half alike; `config.set_row` carries the frame's revision, and a stale or misfit edit is refused with a notice; a blank number input is not an edit of 0.
