...
| I4 | Chat timeline receives ONLY final agent messages; full stream lands in transcript. The turn-end write set (delivery receipt, agent reply, released session) commits in ONE store transaction with the RECEIPT GATING THE REPLY, so no store fault and no daemon death can publish an answer under a receipt that says the turn never landed (peer review 2026-08-08) | e2e: prompt ACP scope, assert timeline row count vs chunk count; plus a store fault injected at the receipt UPDATE asserting zero reply rows, an untouched PENDING leg, and a still-open turn the boot scan then converges | 5 |
...
| I16 | Convergence is not boot-only: an adapter process that dies mid-turn, or a turn that exceeds its deadline, converges at RUNTIME exactly as the boot scan would (turn_interrupted row, delivery resolved terminal, `open_turn_id` cleared, queued prompts given a defined outcome, scope reusable without a daemon restart). Under the multiplexed pool, process death converges EVERY session it hosted, and a per-session deadline cancel never disturbs the process's other sessions (decided 2026-08-04). Convergence can only reach turns the STORE knows about, so a turn whose `open_turn_id` cannot be persisted is never issued at all, failing terminal with detail `turn_unrecorded` (peer review 2026-08-08) | pool test: SIGKILL the shared provider process while TWO scopes have open turns, assert BOTH converge and BOTH accept a new prompt with no restart; deadline test with a fake adapter that never ends one session's turn while a second session's turn completes normally; store fault injected at `set_open_turn` asserting the adapter's own RPC log records no `session/prompt` and the leg resolves terminal | 5, 6 |
...
- [ ] The cap is a CEILING, not a hint (peer review 2026-08-08): capacity is RESERVED before the attach (occupancy = tenants still holding a route + every session mid-attach, so two arrivals racing for one free slot count each other), and an arrival that cannot be given a slot is REFUSED with delivery detail `provider_at_capacity`, terminal and never requeued. Warning and attaching anyway put the process over the configured maximum silently, and did it for every subsequent arrival too, which is unbounded rather than transient. A session already marked `EVICTED` is counted as gone: its route only disappears once its own actor closes the adapter session, and counting it as a tenant would refuse the second of two concurrent arrivals a slot the first had already freed for it
...
- [ ] A turn that cannot be RECORDED is never issued (peer review 2026-08-08): `set_open_turn` runs BEFORE `session/prompt`, and a store failure there fails the leg terminal with detail `turn_unrecorded` instead of prompting anyway. Both convergence paths key off the persisted `open_turn_id` (the deadline sweep queries it, and `converge_dirty_session` writes `acp.turn_interrupted` only for a turn the store knows about), so a turn issued after that write failed is one no sweep can expire and no exit can mark interrupted: a hung adapter holds the scope with a PENDING leg forever. Nothing has reached the adapter at that point (the in-flight permit is dropped and the re-prime prelude is unspent), so FAILED is the honest state and the operator resends
...
- [ ] The turn-end write set is ONE store transaction (peer review 2026-08-08): the delivery receipt, the agent reply and the cleared `open_turn_id` + `state` commit together, and the RECEIPT GATES THE REPLY (a leg another resolver already claimed commits no reply at all, because a timeline answer under an UNKNOWN receipt can never be corrected: the claim is spent). Four independent commits left a daemon death between them showing delivered content under a failed receipt, or a resolved receipt on a session still marked mid-turn, and nothing repaired either. What deliberately stays OUTSIDE that transaction is the transcript's `acp.turn_completed` marker: it is a different table with its own writer, and it is committed BEFORE the transaction so the only surviving crash window leaves a turn the boot scan still lists as dirty and converges (`acp.turn_interrupted` + UNKNOWN + released scope). A transcript that carries both markers is therefore legal and means exactly that: the adapter finished, the daemon did not record it
...
- [ ] The answer receipt's `DELIVERED` means HANDED OFF, and its detail says so (peer review 2026-08-08). ACP defines no acknowledgement for a `session/request_permission` response: `Responder::respond` enqueues the reply on this process's outgoing side and returns, so nothing local can prove the adapter received the decision, let alone applied it. A daemon that dies in that window loses the decision under a receipt that already reads `DELIVERED`, and no wording fixes that; what the vocabulary CAN do is stop implying an ack nobody sent. The operator is not stranded: the attention row is closed at hand-off, the dead adapter's session converges, and the adapter re-asks on its next turn
...
- [ ] Replay suppression is not inferred from a quiet window (peer review 2026-08-08). The seam is armed before `session/load` and stays armed until the pool has DRAINED everything already forwarded and is about to issue the live prompt, in that order: the transport preserves order into the demux channel (the upstream connection runs every notification handler inline on one event-loop task, so a notification the adapter wrote before its load reply is queued before that reply resolves), but the pool's own supervisor forwards to the session on a DIFFERENT task, so a replay tail can land in the session's channel while the turn is being recorded. A quiet window cannot see that; a drain immediately before the seam closes swallows it whatever the store's write latency was. WHAT THIS DOES NOT COVER, stated plainly: an adapter that emits replay AFTER its own `session/load` reply and after the next prompt was issued is indistinguishable from live output, because ACP v1 gives a `session/update` no turn identity and no replay marker. The guarantee is therefore "everything the adapter sent before its load reply is suppressed, however late the daemon reads it", not "no replay row can ever be written"
