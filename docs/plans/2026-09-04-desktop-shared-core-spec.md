...
**Amendment (2026-09-19, D3-prime): the plugin fallback cell paints cells from a runtime the Tauri shell owns, on a channel that is not a section.** The desktop runs no plugin runtime (`ainb-desktop/src/executor.rs`, `NO_PLUGIN_RUNTIME`), and `ainb-app` owns none by test. The cell does not change either: the Tauri shell, a process boundary outside `ainb-app`, owns an `ainb_plugin_runtime::RuntimeHandle`, the same crate the terminal brings up, started on the first `plugin.owned.watch_screen` and stopped with the window, and the desktop's own executor delivers `ForwardToPlugin` and `RunPluginAction` to it. The watchable set is `PLUGIN_SCREENS` less `analytics`: the stats tab draws burndown's counters natively, and a cell blitting burndown's paint beside it would put the same numbers on the desktop twice, from the two scans the amendment above records. Painted `WireBuffer` cells cross to the webview on a Tauri `Channel` of their own, the pattern the terminal's bytes use: never on a section, never persisted, never off-box, which is why no key-path fixture claims them and why no plugin JSON rides a frame. The webview paints them into a plain cell grid; a terminal emulator is not asked to be a cell blitter. The channel is bounded twice, at 30 frames a second per watched screen coalescing to the newest buffer, and at a viewport of 300 by 100 with a larger buffer clamped and the clamp counted, so a plugin that paints faster or larger than the window cannot fill it. Failure is drawn, not logged: a plugin that exits shows the render-error placeholder with the scrubbed reason from `PluginsHostView` and is restarted on a backoff from one second doubling to thirty, at most five times, after which the placeholder says it is stopped and offers one start action; a plugin that never paints shows the no-frame placeholder after the lease. The bundle carries an allow-list of plugin binaries beside the daemon sidecar in `tauri.conf.json`, one per screen in the watchable set, and the runtime is handed that list rather than a directory to discover, so nothing outside the bundle is executed. The three placeholders draw from `PluginsHostView`, which is framed and scrubbed already; only the live cells need the host.

**Amendment (2026-09-19, D3-prime): what "full parity suite" admits for the screens this node draws.** A screen that exists on both surfaces has a fixture drawn by both renderers against one expected-facts list: `inbox`, and `plugins_host` for the three placeholder states a plugin screen draws with no runtime, no plugin, or no frame. A screen that exists on one surface only has a fixture on that half, with the reason recorded: `stats` is a DOM-half fixture over section 21, because the TUI's stats is burndown's plugin paint on `analytics` and a second built-in beside it would be drift. A stubbed half never passes: the mutation check that D3 lands (one fact deleted from one renderer's output fails the suite) is re-proved on `inbox`. The full enumeration, every screen covered on both halves, on one, or excluded with its reason, lives in the D3-prime goal and is the definition of the gate; a screen added to either surface without a row there is a failed criterion.
