...
**Amendment (2026-09-19, D3-prime): the plugin fallback cell's live half, the runtime the shell owns and the channel its cells ride.** The second half of the cell, built only once this amendment is accepted. The Tauri shell, a process boundary outside `ainb-app`, owns an `ainb_plugin_runtime::RuntimeHandle`, the crate the terminal brings up, started on the first `plugin.owned.watch_screen` for a screen in the watchable set and stopped with the window; the desktop's own executor delivers `ForwardToPlugin` and `RunPluginAction` to it, and `ainb-app` still owns nothing, by the test that already says so. The watchable set is `PLUGIN_SCREENS` less `analytics`: the stats tab draws burndown's counters natively, and a cell blitting burndown's paint beside it would put the same numbers on the desktop twice. Painted `WireBuffer` cells cross to the webview on a Tauri `Channel` per watched screen, the pattern the terminal's bytes use: never on a section, never persisted, never off-box, which is why no key-path fixture claims them and no plugin JSON rides a frame. The webview paints them into a plain cell grid; a terminal emulator is not asked to be a cell blitter. The channel is bounded four ways. In frames: at most 30 a second per screen, coalescing to the newest buffer when the webview is behind. In cells: a viewport of 300 columns by 100 rows, a larger buffer clamped and the clamp counted on the frame. In bytes: each frame is encoded as a style table followed by run-length cells (one style index and one text run per run of equal style), so a 300 by 100 buffer of ordinary text is tens of kilobytes rather than a megabyte, and a frame past `MAX_CELL_FRAME_BYTES` (256 KiB, four times a plausible worst case) is not sent: the previous frame stays and the drop is counted, so a plugin painting noise cannot fill the channel. In flight: the channel reuses the terminal's credit window (`ainb-desktop/src/terminal.rs`, `WINDOW_BYTES`, 4 MiB acked after paint) per watched screen, so no frame is sent without credit, a blocked screen coalesces newest-wins, and four watched screens hold at most four windows unacked. Failure is drawn, not logged: a plugin that exits shows the render-error placeholder with the scrubbed reason from `PluginsHostView` and is restarted on a backoff from one second doubling to thirty, at most five times, after which the placeholder says it is stopped and offers one start action; a plugin that never paints shows the no-frame placeholder after the lease. The bundle carries an allow-list of plugin binaries beside the daemon sidecar in `tauri.conf.json` (`bundle.externalBin`), one per screen in the watchable set (`witr`, `learnings`, `abtop`, the hangar plugin), and the runtime is handed that list rather than a directory to discover, so nothing outside the bundle is executed. What that costs the release: D4-prime's matrix (its goal, D4b, `docs/plans/goals/2026-09-19-d4-updater-release.md`) stages one sidecar per target today; with this amendment it stages five per target, for `aarch64-apple-darwin`, `x86_64-apple-darwin` and `x86_64-unknown-linux-gnu`, each asserted by `--version` in the bundle smoke, each signed and notarised with the Developer ID when `desktop_signed` is true and ad-hoc signed otherwise, so every bundle grows by four binaries and every signing pass by four items per identity. D4b's amendment records that count against its matrix before the first plugin binary is staged; until then, the bundle carries the daemon alone and this half of the cell is not built.
