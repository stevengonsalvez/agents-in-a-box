...
| `mcp-import.gif` | `mcp-import.tape` | The overlay's **import** action (`i`): open the pool overlay on an empty pool, press `i`, and the project's `.mcp.json` server is written to `./.ainb/config.toml` + registered live — a `▸ imported …` result line appears and the server row shows. Drives `scripts/mcp-import-journey.sh`. |
