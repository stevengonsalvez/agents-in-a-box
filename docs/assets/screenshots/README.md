...
| `mcp-overlay.gif` | `mcp-overlay.tape` | The in-TUI pool overlay (sidebar MCP / `m`): a live table showing context7 `shared ×2` with both session names, plus the stop/refresh action bar. Drives `scripts/mcp-overlay-journey.sh`. |
