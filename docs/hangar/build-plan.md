...
| ✅ P5 | W11-12 | Auth + workspace + secret store | OS keychain integration for LLM keys; allowlist env enforcement; workspace switching — **DONE** (6 beads: `ainb-hangar-secrets` keychain crate; `host/secret_store_get` full cutover to `{scope,key}` + `secrets:read` cap; env allowlist w/ hardcoded deny + `hangar config env.allow` CLI; PAT/daemon_token sha256-only + constant-time verify; workspace switching in Settings + `state.toml` + `WorkspaceChanged`; danger-access modal + first-run e2e. Security fix: list-form `workspace:write` rejected `-32003`. Per-provider live-render deferred to P8 event bus) |
| ✅ P6 | W13-14 | Skills + curated templates | Embedded `agent_template` JSONs; `ainb hangar skills sync` importer from `toolkit/packages/skills/` — **DONE** (5 beads + IDOR security fix: SkillRepo CRUD w/ workspace scoping; `skills sync` importer (all-or-nothing, idempotent); 10 curated templates embedded via `build.rs` skill-ref guard + `templates list/show/use`; dispatch-time materialisation into provider-native skill paths (`.claude/skills/` etc.); skill manager screen + 5 daemon RPCs + capstone e2e (sync→use→assign→dispatch→materialise byte-asserted). Security: by-id SkillRepo methods workspace-scoped, `CrossWorkspace` on attach. Follow-ups: `4pe` IDOR ✅; `sentry-cli` malformed-skill sync-abort policy (low-pri); per-provider warning live render → P8) |
| ✅ P7 | W15-16 | Autopilots + cron scheduler | Schedule-only autopilots; daemon scheduler thread; autopilot manager screen — **DONE** (6 beads: cron parser (5→6 field normalise, DST/leap-safe); autopilot CRUD + migration 0009 (workspace-scoped by-id); `fire_autopilot_tick` single-tx run+task + migration 0010 link + FSM completion cascade; scheduler thread (wake-handle `select!`, fire-time concurrency skip + `tick_skipped` audit); manager screen + 4 RPCs; capstone e2e (fires-on-schedule + skips-when-running, FixedClock advance + scheduler wake) + `ainb hangar autopilot` CLI). |
| ✅ P8 | W17-18 | Kanban + Daemon health + observability polish | Kanban board screen; daemon health pane; tracing + OTEL exporter wired — **DONE** (7 beads: tracing-subscriber + rolling JSONL sink (`daemon.<date>`); OTLP exporter (optional `otlp` feature, zero otel in default build, env-discovered + graceful shutdown); `#[instrument]` spans on 8 service methods (task FSM / autopilot tick / beads sync); Kanban 4-col board + `tasks_list`/`task_transition` RPCs; daemon health pane + dual-dim sparkline + ring-buffer collector; `ainb hangar logs tail` CLI + TUI logs screen; 3 e2e tripwires (kanban + health-sparkline via tmux, otel-export) 5×-flake-free. K/D/L hotkeys wired.) |
