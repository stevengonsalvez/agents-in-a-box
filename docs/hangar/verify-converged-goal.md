...
| `journeys/t1-card-create-parity.gif` | **T1 — task-create parity (this update).** The tcp T1 worktree-lifecycle journey (F1-F5): Boards `c` → title → repo stage — an unpicked Enter is REFUSED (`REQUIRED`, the `@` dropdown reopens on `scratch` rather than advancing repo-less) → `@`-filter narrows/restores the scanned `testrepo` live → pick it → agent chips cascade-preselect `claude` → profile `[claude-agent]` → create → `Run ▾` Headless. While the card runs, a real `git` worktree lives at `~/.agents-in-a-box/worktrees/<slug>` on branch `ainb/<slug>` (shell-verified via `git branch --list` alongside the on-screen still); on completion the card auto-moves to Done (`✓`) and the clean worktree is torn down. |
| `journeys/t4-squad-fanout.gif` | **T4a — squad-card fan-out (this update).** A card assigned to the `shippers` squad (leader `claude-agent` + members `member-one`/`member-two`) fans out on Run (`Enter` → `Run ▾` → `Enter`, Headless) into a leader task + two member tasks; the card's title grows a live `👥` chip per member (`claude-agent:running member-one:running member-two:running`). Shell-verified while the fan-out is held open: three distinct worktree directories exist simultaneously under `~/.agents-in-a-box/worktrees/<slug>`, each matching a registered `ainb/<slug>` branch. Releasing the agents finalizes all three to done, auto-moves the card to Done, and tears every worktree down cleanly. |
| `journeys/t4-card-deps.gif` | **T4b — card-dependency chain (this update).** Card B (`DepDependentB`) depends-on card A (`DepBlockerA`) with auto-run on, rendering `🔒[-dep-a]` + `⏵` in Todo. Focusing B and committing Run is REFUSED — the board header reads `run failed: this card is blocked by unfinished cards (-dep-a); finish them (or remove the dependency) first` — and B gains no task (shell-verified: zero rows the entire time A is pending/running). Running A instead launches it headless; the moment A finalizes to done the finalize seam AUTO-LAUNCHES B, and both cards land in Done with a `✓`. |
| `journeys/t5-notify-routing.gif` | **T5 — notification routing (this update).** Settings → Notifications grid shows the seeded `ask_user_question` default (web+os); `space` on phone enables it and `space` on os disables it, leaving phone+web — a real `hangar/notify_rule_set` round trip, visible on screen as the grid's flipped state. A real ASK is then raised through the hook seam (an `AskUserQuestion` transcript + a `Notification` line appended to the real `events.jsonl`), and the raised row is shell-verified to carry `channels=phone,web` — not the seeded `web,os` default — before Control Center shows the routed ASK (`1 need you`, the question + its `①②` options). Scope note: the grid's toggle writes a WORKSPACE-scoped override, but hook-ingest always resolves against the GLOBAL rule (`notify::resolve_channels(pool, kind, None)`, hardcoded by design) — so this recording also flips the GLOBAL row via the same RPC with no `workspace_id`, exactly like the tripwire's own acceptance, to make the live raise honour the on-screen change. |
