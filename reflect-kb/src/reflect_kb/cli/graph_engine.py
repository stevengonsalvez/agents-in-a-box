...
class LearningsGraphEngine:
    ...
    def __init__(
        ...
        # Opt-in shared Postgres backend (ainb-reflect-memory). When BOTH a DSN
        # and a workspace id resolve — from args or env — nano-graphrag's graph,
    ...
    def _init_graph(self):
        ...
        # Shared Postgres backend (opt-in). When configured, hand nano-graphrag
        # the ainb-reflect-memory storage classes so its graph / vectors /
        # community reports live in shared Postgres — same store across
        # machines. nano-graphrag's own code is unchanged; we only swap the
        # *_storage_cls and pass connection + tenant via addon_params.
        if self._pg_dsn and self._workspace_id:
            try:
                from ainb_reflect_memory.nanographrag import (
                    addon_params,
                    storage_classes,
                )
            except ImportError as exc:
                raise GraphEngineError(
                    "Postgres backend requested (REFLECT_PG_DSN/DATABASE_URL + "
                    "REFLECT_WORKSPACE_ID) but ainb-reflect-memory is not "
                    f"installed: {exc}"
                )
            ...
            logger.info(
                "LearningsGraphEngine: using shared Postgres backend "
                "(workspace=%s)",
                self._workspace_id,
            )
